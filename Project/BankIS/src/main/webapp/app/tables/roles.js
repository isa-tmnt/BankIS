...
app.component('roles', {
    ...
    controller: ['$scope', '$http', '$attrs', '$rootScope', function CurrensiesCtrl($scope, $http, $attrs, $rootScope) {
        ...
        $scope.setSelected = function (row) {
            if ($attrs.iamdialog)
                $rootScope.$broadcast('ROLE_SELECTED', row);
            $scope.selected = row;
        ...
        $scope.doAdd = function () {
            ...
            $http.post(appConfig.apiUrl + 'roles', $scope.editing, appConfig.config).then(function successCallback(response) {
        ...
        $scope.doEdit = function () {
            if ($scope.selected.id) {
                $http.put(appConfig.apiUrl + 'roles', $scope.editing, appConfig.config).then(function successCallback(response) {
                    var row = response.data;
                    if (row && response.status < 300) {
                        $scope.header.filter(h => h.type == "date").forEach(h => row[h.code] = new Date(row[h.code]));  //conver strings to dates where needed
        ...
        $scope.doRemove = function () {
            if ($scope.selected.id) {
                $http.delete(appConfig.apiUrl + 'roles/' + $scope.selected.id, appConfig.config).then(function successCallback(response) {
