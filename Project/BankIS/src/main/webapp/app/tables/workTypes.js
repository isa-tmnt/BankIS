...
app.component('workTypes', {
    ...
    controller: ['$scope', '$http', '$attrs', '$rootScope', '$routeParams', function WorkTypeCtrl($scope, $http, $attrs, $rootScope, $routeParams) {
        ...
        $http.get(appConfig.apiUrl + 'worktypes', appConfig.config).then(function successCallback(response) {
        ...
        $scope.doAdd = function () {
            ...
            $http.post(appConfig.apiUrl + 'worktypes', $scope.editing, appConfig.config).then(function successCallback(response) {
        ...
        $scope.doRemove = function () {
            if ($scope.selected.id) {
                $http.delete(appConfig.apiUrl + 'worktypes/' + $scope.selected.id, appConfig.config).then(function successCallback(response) {
        ...
        $scope.nexts = [
            { label: "Legal Person Details", link: "/#!/legalPersonDetails", filterProperty: "workType" },
        ];
